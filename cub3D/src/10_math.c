...
/*   Updated: 2023/10/10 11:42:01 by hongbaki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_pt2d_d	calc_ray_dir(int x, t_pt2d_d raydr, t_pt2d_d dir, t_pt2d_d plane)
{
	...
	//Calculate camera_x based on the x-coordinate and the window width
...
//it sets the corresponding delta distance to a large value to avoid division by zero
t_pt2d_d	calc_delta_dist(t_pt2d_d delta_dist, t_pt2d_d raydr)
{
	...
	// based on the reciprocal of the x-component of the raydr vector
	else
...
void	calc_step_and_side_dist(cub_main *cub)
{
	...
	if (cub->raydr.x < 0)
	{
		...
		// check current player's position of the ray and grid(map) position.
		// side_dist is for the adjacent grid position on the positive x side
		cub->side_dist.x = (cub->pos.x - cub->map_pos.x) * cub->delta_dist.x;
	}
	else
	{
		cub->step.x = 1;
		cub->side_dist.x = (cub->map_pos.x + 1.0 - cub->pos.x) * cub->delta_dist.x;
	}
	if (cub->raydr.y < 0)
	{
		cub->step.y = -1;
	...
	{
		cub->step.y = 1;
		cub->side_dist.y = (cub->map_pos.y + 1.0 - cub->pos.y) * cub->delta_dist.y;
	}
}
...
//It calculates the intersection of a ray with walls in the environment 
//and determines which side (horizontal or vertical) of the wall was hit
